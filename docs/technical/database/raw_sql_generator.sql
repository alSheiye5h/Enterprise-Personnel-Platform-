-- ============================================================================
-- SYSTÈME DE GESTION RH - CONFORME AU CADRE LÉGAL MAROCAIN
-- Version : 2.0
-- Dernière mise à jour : Conformité Code du Travail, CNSS, AMO, IGR
-- ============================================================================

-- Table principale des employés avec toutes les validations légales
CREATE TABLE employe (
    -- Identifiants uniques (contraintes légales)
    code_employe VARCHAR(20) PRIMARY KEY GENERATED ALWAYS AS (
        CASE 
            WHEN pays = 'Maroc' THEN CONCAT('MA-EMP-', LPAD(nextval('employe_seq')::TEXT, 6, '0'))
            ELSE CONCAT('INT-EMP-', LPAD(nextval('employe_seq')::TEXT, 6, '0'))
        END
    ),
    
    -- === DOCUMENTS D'IDENTITÉ (conformité légale) ===
    cin VARCHAR(20) CHECK (
        -- Règle 1 : CIN obligatoire pour Marocains
        (pays = 'Maroc' AND cin IS NOT NULL AND LENGTH(cin) = 12)
        OR (pays != 'Maroc' AND cin IS NULL)
    ),
    numero_passeport VARCHAR(20) CHECK (
        -- Règle 2 : Passeport obligatoire pour étrangers
        (pays != 'Maroc' AND numero_passeport IS NOT NULL)
        OR (pays = 'Maroc' AND numero_passeport IS NULL)
    ),
    
    -- === INFORMATIONS CNSS/AMO (conformité obligatoire) ===
    numero_immatriculation_cnss VARCHAR(20) NOT NULL,
    date_immatriculation_cnss DATE NOT NULL,
    numero_assure_amo VARCHAR(30),
    statut_cotisation_cnss VARCHAR(20) DEFAULT 'ACTIF' CHECK (statut_cotisation_cnss IN ('ACTIF', 'SUSPENDU', 'RADIE')),
    ...
    situation_matrimoniale VARCHAR(15) NOT NULL CHECK (
        situation_matrimoniale IN ('CELIBATAIRE', 'MARIE', 'DIVORCE', 'VEUF')
    ),
    nombre_enfants INTEGER DEFAULT 0 CHECK (nombre_enfants >= 0),
    nb_personnes_charge INTEGER NOT NULL DEFAULT 0 CHECK (nb_personnes_charge >= 0),
    ...
    -- === ADRESSE COMPLÈTE (obligatoire pour déclarations légales) ===
    ...
    -- === COORDONNÉES BANCAIRES (pour virement obligatoire) ===
    ...
    categorie_professionnelle VARCHAR(50) CHECK (
        -- Conformité à la classification du Code du Travail
...
-- Index pour performances et contrôles
...
-- ============================================================================
-- TABLE : CONTRAT DE TRAVAIL (Conformité Code du Travail Marocain)
-- ============================================================================
...
CREATE TABLE contrat_travail (
    ...
    date_fin_contrat DATE CHECK (
        ...
        (type_contrat != 'CDD') OR 
        (type_contrat = 'CDD' AND date_fin_contrat <= date_debut_contrat + INTERVAL '24 months')
    ),
    date_signature DATE NOT NULL,
    date_effet DATE NOT NULL,
    
    -- === PÉRIODE D'ESSAI (Art. 14 Code du Travail) ===
...
-- ============================================================================
-- TABLE : FICHE DE PAIE (Conformité CNSS, AMO, IGR)
-- ============================================================================
...
CREATE TABLE fiche_paie (
    ...
    salaire_brut DECIMAL(10,2) GENERATED ALWAYS AS (
        ...
        prime_anciennete + prime_panier + prime_transport + prime_logement +
        prime_presence + prime_performance + commission + autres_primes +
...
-- ============================================================================
-- TABLE : CONFIGURATION FISCALE ET SOCIALE (Paramètres légaux)
-- ============================================================================
...
CREATE TABLE configuration_legale (
    ...
    -- === BARÈME IGR ===
    bareme_igr JSONB NOT NULL CHECK (jsonb_typeof(bareme_igr) = 'array'),
...
-- Table de liaison Employé-Département (allocation multi-départements)
...
CREATE TABLE audit_conformite (
    ...
    taux_conformite DECIMAL(5,2) GENERATED ALWAYS AS (
        CASE 
            ...
            ELSE GREATEST(100 - (nombre_anomalies * 5), 0) -- Calcul simplifié
        END
    ) STORED,
...
-- ============================================================================
-- VUES UTILES POUR LES RAPPORTS LÉGAUX
-- ============================================================================

-- Vue pour le registre du personnel (obligatoire)
...
-- Vue pour l'état annuel du personnel (déclaration fiscale)
...
-- ============================================================================
-- COMMENTAIRES SUR LES CONTRAINTES LÉGALES
-- ============================================================================

COMMENT ON TABLE employe IS 'Table principale des employés conforme au Code du Travail Marocain. Contient toutes les validations légales (âge minimum, CIN/passeport, etc.)';
COMMENT ON COLUMN employe.cin IS 'Carte d''Identité Nationale obligatoire pour les Marocains (12 chiffres)';
COMMENT ON COLUMN employe.date_naissance IS 'Doit garantir un âge minimum de 18 ans (Code du Travail Art. 143)';
COMMENT ON COLUMN employe.numero_immatriculation_cnss IS 'Numéro d''immatriculation CNSS obligatoire pour tout salarié';
COMMENT ON COLUMN employe.rib IS 'RIB marocain obligatoire pour le virement bancaire des salaires';

COMMENT ON TABLE contrat_travail IS 'Contrats de travail conformes aux articles 14 à 62 du Code du Travail Marocain';
COMMENT ON COLUMN contrat_travail.periode_essai_jours IS 'Période d''essai maximale: 90 jours (non-cadres) ou 180 jours (cadres) - Art. 14';
COMMENT ON COLUMN contrat_travail.heures_hebdomadaires IS 'Durée légale du travail: maximum 44 heures/semaine - Art. 184';
COMMENT ON COLUMN contrat_travail.jours_conges_acquis IS 'Droit aux congés: 1.5 jours ouvrables par mois de travail effectif - Art. 234';

COMMENT ON TABLE fiche_paie IS 'Fiches de paie conformes CNSS/AMO/IGR avec calculs automatiques selon la réglementation marocaine';
COMMENT ON COLUMN fiche_paie.assiette_cnss IS 'Assiette CNSS plafonnée à 6,000 MAD (valeur pratique 2024)';
COMMENT ON COLUMN fiche_paie.allocation_familiale IS 'Allocation familiale CNSS: 150 MAD par personne à charge';
COMMENT ON COLUMN fiche_paie.heures_supp_25 IS 'Heures supplémentaires 44-48h: majoration de 25%';
COMMENT ON COLUMN fiche_paie.heures_supp_50 IS 'Heures supplémentaires >48h et jours repos: majoration de 50%';
COMMENT ON COLUMN fiche_paie.heures_supp_100 IS 'Heures jours fériés: majoration de 100%';

COMMENT ON TABLE declaration_cnss IS 'Déclarations mensuelles CNSS obligatoires (avant le 10 du mois suivant)';
COMMENT ON TABLE configuration_legale IS 'Paramètres légaux annuels (SMIG, taux CNSS/AMO, barème IGR) - Mise à jour annuelle obligatoire';